...
//paker CODE
var PACKER_NONE = 0;
var PACKER_MINIMIZE = 1;
var PACKER_COMPRESS_BASE62 = 2;
var PACKER_COMPRESS_SHRINK = 3;
var PACKER_COMPRESS_BASE62_SHRINK = 4;
...
var code_prefix	= new String("NODEJS_");
var dump_nfa	= false;
var dump_dfa	= false;
var verbose		= false;
var	dfa_table;
var packer = PACKER_NONE;


var argv = process.argv;
for( var i = 2; i < argv.length; i++ )
{
	if( argv[i].toLowerCase() == "-m"
			|| argv[i].toLowerCase() == "--minimize" )
		packer = PACKER_MINIMIZE;
	else if( argv[i].toLowerCase() == "-c"
			|| argv[i].toLowerCase() == "--compress" )
		packer = PACKER_COMPRESS_BASE62;
	else if( argv[i].toLowerCase() == "-s"
			|| argv[i].toLowerCase() == "--shrink" )
		packer = PACKER_COMPRESS_SHRINK;
	else if( argv[i].toLowerCase() == "-cs"
			|| argv[i].toLowerCase() == "--shrink-compress" )
		packer = PACKER_COMPRESS_BASE62_SHRINK;
	else if( argv[i].toLowerCase() == "-o"
	...
	if( errors == 0 )
	{
		...
		if( errors == 0 )
		{
			...
			{		
				...
				switch(packer){
					case PACKER_MINIMIZE:
						driver = require('packer').pack(driver);
						break;
					case PACKER_COMPRESS_BASE62:
						driver = require('packer').pack(driver,true);
						break;
					case PACKER_COMPRESS_SHRINK:
						driver = require('packer').pack(driver,false,true);
						break;
					case PACKER_COMPRESS_BASE62_SHRINK:
						driver = require('packer').pack(driver,true,true);
						break;
					case PACKER_NONE:
					default:
						break;
